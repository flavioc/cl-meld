...
(defun set-type (expr typs)
   (cond
      ...
      ((or (call-p expr) (op-p expr)) (setf (cdddr expr) (list (try-one typs))))))
   ...
   (mapcar #'(lambda (arg)
                  (cond ((const-p arg)
                           (letret (new-var (generate-random-var))
                              (let ((new-constraint (make-constraint (make-equal new-var '= arg))))
                                 (push new-constraint (clause-body clause)))))
                         (t arg))) args))
