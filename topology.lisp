   ...
   (cond
    ((string-equal (data-input-template input) "stanford-snap")
     (let ((ret (snap-file-read (data-input-file input))))
        ...
        ret))
    ((string-equal (data-input-template input) "stanford-sssp-snap")
     (let ((ret (snap-sssp-file-read (data-input-file input))))
      (setf *nodes* (data-input-nodes ret))
      ret))
    ((string-equal (data-input-template input) "stanford-snap-undirected")
     (let ((ret (snap-undirected-file-read (data-input-file input))))
      (setf *nodes* (data-input-nodes ret))
      ret))
    ((string-equal (data-input-template input) "stanford-snap-undirected")
       (let ((ret (snap-undirected-file-read (data-input-file input))))
         (setf *nodes* (data-input-nodes ret))
         ret))
    ((string-equal (data-input-template input) "stanford-snap-pagerank")
      (let ((ret (snap-pagerank-file-read (data-input-file input))))
       (setf *nodes* (data-input-nodes ret))
       ret))
    ((string-equal (data-input-template input) "stanford-snap-basic")
     (let ((ret (snap-basic-file-read (data-input-file input))))
      (setf *nodes* (data-input-nodes ret))
      ret))
